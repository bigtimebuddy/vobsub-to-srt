...
class VobSubDecoder {
  ...
  parseTimestamp(timestamp) {
    const [hours, minutes, seconds, milliseconds] = timestamp
      .split(":")
      .map(Number);
    return hours * 3600000 + minutes * 60000 + seconds * 1000 + milliseconds; // milliseconds are already in correct format
  }
   ...
   */
  async processFrames(subtitlesWithImages, quality = "fast") {
    ...
    try {
      ...
      // First pass: collect frames with text
      const framesWithText = [];
      for (let i = 0; i < subtitlesWithImages.length; i++) {
        ...
        try {
          ...
          if (text && text.trim().length >= 2) {
            ...
            framesWithText.push({
              frameIndex: subtitle.frameIndex || i + 1,
              text: wrappedText,
              originalTimestamp: subtitle.timestamp,
            });
          } else {
            if (this.verbose)
              console.log(
      ...
      // Second pass: create SRT entries using sequential subtitle timings
      const srtEntries = [];
      for (let i = 0; i < framesWithText.length; i++) {
        const frame = framesWithText[i];

        // Use sequential subtitle timing entries instead of frame-based timing
        const subtitleEntry = this.subtitles[i];
        const nextSubtitleEntry = this.subtitles[i + 1];

        if (subtitleEntry) {
          const startTime = subtitleEntry.timestamp;
          const endTime = nextSubtitleEntry
            ? nextSubtitleEntry.timestamp
      ...
      if (this.verbose)
        console.log(
          `Completed batch OCR processing of ${subtitlesWithImages.length} frames, generated ${srtEntries.length} subtitle entries`,
        );
      return srtEntries;
    } catch (error) {
